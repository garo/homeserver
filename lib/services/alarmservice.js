...
exports.triggerAlarm = function (alarm) {
  ...
  if (alarm.zabbix_key) {
    var cmd = '/usr/bin/zabbix_sender -s ' + os.hostname() + " -c /etc/zabbix/zabbix_agentd.conf -k " + alarm.zabbix_key + " -o 1";
...
exports.clearAlarm = function (alarm) {
  ...
  if (alarm.zabbix_key) {
    var cmd = '/usr/bin/zabbix_sender -s ' + os.hostname() + " -c /etc/zabbix/zabbix_agentd.conf -k " + alarm.zabbix_key + " -o 0";
    console.log("cmd: ", cmd);
    exec(cmd, function (err) {
      if (err) {
        console.error("Err on zabbix_sender", err);
