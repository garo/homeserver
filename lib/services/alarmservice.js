...
exports.init = function (cb) {
  ...
  setInterval(exports.resetState, 60000 * 5);
  cb();
};
...
/**
 * This resets the alarm state so that the program will re-send all alarms
 * once this is called.
 */
...
function triggerAlarmAfterCheck(err, pin, check, allowAlarm) {
  if (allowAlarm) {
    alarmStates[pin] = 1;
    ...
    if (alarmChecks.hasOwnProperty(pin)) {
      ...
      if ((((inputs) & (1 << (pin))) && alarmChecks[pin].triggeron === "high") ||
        ...
        if (alarmStates[pin] !== 1) {
          ...
          if (check.extracheck) {
            check.extracheck(pin, check, triggerAlarmAfterCheck);
